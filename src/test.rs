...
mod tests {
    ...
    fn test_frostbolt_cooldown() {
        ...
        let mut enemy = make_enemy(1000.0);
        mage.use_ability("frostbolt", &mut enemy);  // cast 1 — cd = 3

        let mana_after_first = mage.stats.current_mana;
        mage.use_ability("frostbolt", &mut enemy);  // blocked
        assert_eq!(mage.stats.current_mana, mana_after_first);
        ...
        mage.use_ability("frostbolt", &mut enemy);  // OK
        assert!(mage.stats.current_mana < mana_after_first);
    }
